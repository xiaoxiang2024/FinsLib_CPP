...
/*
* 尝试通过FINS与远程FINS TCP服务器连接。 如果连接成功，
* 则返回一个指向包含连接数据的系统结构的指针。 否则，返回值为NULL。
* 简单模式
* 传入FINS设备指针，远程设备IP，端口号，远程设备节点号，错误代码指针和最大错误数量（若错误数超过该值则会返回达到错误上限的错误）
*/
...
* 则返回一个指向包含连接数据的系统结构的指针。 否则，返回值为NULL。
* 正常模式
* 传入FINS设备指针，远程设备IP，端口号，本地网络号，本地节点号，本地单元号，远程设备网络号，远程设备节点号，远程设备单元号，错误代码指针和最大错误数（若错误数超过该值则会返回达到错误上限的错误）
*/
...
*
* 关闭FINS套接字并根据前一个操作返回的错误设置一个错误值。
* 传入参数为FINS设备指针，错误代码指针
*/
...
*
* 关闭客户端TCP连接的FINS套接字。
*  它首先将读取和发送的超时设置为零，否则停止套接字可能需要无限期的时间。 
*  它还重置错误计数器和重试的全局超时值。 指针返回一个指向系统结构的指针，或者在发生错误时返回NULL。
*/
...
*
* 远程连接的PLC接收信息，该信息通过网络以FINS协议发送。 
* 只要有新的信息进入，数据就会附加到缓冲区并继续接收数据。
*/
...
* 检查连接上的错误代码和当前错误计数。 
* 如果错误代码指示成功，则计数器复位为0。
* 否则，如果计数器达到最大错误计数，则计数器复位并且连接关闭。 
* 在这种情况下，函数返回最大错误计数错误。 
* 否则，错误表示为FINS_RETVAL参数。
*/
static int check_error_count(struct fins_sys_tp* sys, int error_code)
{
	if (sys == NULL) return FINS_RETVAL_NOT_INITIALIZED;
...
* 
*通过TCP连接将命令头和主体发送到远程PLC。 TCP头应该已经发送过。
*/
...
* static int tcp_errorcode_to_fins_retval( uint32_t errorcode );
*
* 将FINS / TCP头部错误代码转换为FINS_RETVAL，FINS_RETVAL被调用例程识别。
*/
 ...
 *
 * 从远程PLC接收TCP响应头。 该函数检查返回的标题的完整性。 
 * 如果发生错误，则返回-1。 否则，返回值是以下有效负载的大小。 
 * 请注意，在某些情况下，有效负载长度可能为0，这不是错误。
 */
...
*
* 从远程PLC接收命令结构。 字节数长度作为参数提供。 这个长度包括完整的头和一些正文。
*/
...
*
* 外部程序用于执行与FINS服务器的实际通信的功能。 
* 该功能既发送命令又接收响应并隐藏调用例程的低级通信的所有细节。
...
* 将一个Windows WSA错误代码转换为FINS_RETVAL_值。
*/
